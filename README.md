...
2.  Call predict() on the trained random forest model to obtain
...
- **predicted_m6A_status**: “Positive” or “Negative”, based on the
  threshold.

### **4.Model Performance (ROC and PRC Curves)**

To demonstrate the strong predictive power of the random forest model,
the following figures show the ROC and PRC curves generated in
Practical4.

**ROC curves:**

The ROC curve below illustrates the trade-off between the true positive
rate (recall) and the false positive rate (FPR) across different
prediction thresholds.  
The model demonstrates a strong ability to distinguish m6A-positive and
m6A-negative sites, with a consistently increasing recall as FPR grows.

![](man/figures/roc_curve.png)

**PRC curves:**

The PRC curve provides additional insight under class-imbalanced
settings, which is common in biological datasets.  
The model maintains high precision at moderate recall levels, indicating
that most predicted positives are true positives.  
Although precision naturally decreases at very high recall, overall
performance remains strong, demonstrating the classifier’s reliability
